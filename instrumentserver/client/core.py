...
from .. import DEFAULT_PORT, QtCore
from ..base import send, recv, recvMultipart
                ...
                if err is not None:
                    if isinstance(err, str):
                        ...
                        if self.raise_exceptions:
                            ...
                            logger.error(
                                f"Server raised the following exception: {err}"
                            )
                    else:
                        if self.raise_exceptions:
...
class Client(BaseClient):
    ...
    def getParamDict(
        self, instrument: str = None, attrs: List[str] = ["value"], *args, **kwargs
    ):
        msg = ServerInstruction(
